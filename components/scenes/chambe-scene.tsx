...
interface ChambeSceneProps {
  onComplete: () => void
  isActive: boolean
}

const videoUrl = "/video/aa5.mp4"

export function ChambeScene({ onComplete, isActive }: ChambeSceneProps) {
  ...
  useEffect(() => {
    if (secondTextComplete && isActive) {
  ...
  }, [secondTextComplete, isActive, onComplete]);
